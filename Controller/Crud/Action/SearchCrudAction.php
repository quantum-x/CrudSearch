 ...
 */
class SearchCrudAction extends CrudAction {
 ...
 * @return void
 */
	protected function _get($search_term = null) {
		$model = $this->_model();

		$findMethod = $this->_getFindMethod('search');

		$query = array();
		$query['conditions'] = array($model->displayField => $search_term);
		$query['recursive'] = -1;
		$query['limit'] = 6;
		...
		if (empty($item)) {
			...
			$message = $this->message('recordNotFound', array('id' => $search_term));
			$exceptionClass = $message['class'];
			throw new $exceptionClass($message['text'], $message['code']);
		}

		//Cleanup the returned arrays
		$finalItems = [];
		foreach($item as $i) $finalItems[] = $i[$model->name];
		$item = $finalItems;
